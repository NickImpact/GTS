	...
	private void setupDesign(){
		...
		if(admin){
			...
			admin.addListener(ClickInventoryEvent.class, e -> {
                ...
                Sponge.getScheduler().createTaskBuilder().execute(p::closeInventory
            ...
            if(lot.getLot().getOwner().equals(player.getUniqueId())){
				...
				confirm.addListener(ClickInventoryEvent.class, e -> {
                    ...
                    Sponge.getScheduler().createTaskBuilder().execute(p::closeInventory).delayTicks(1).submit(GTS.getInstance());
                });
