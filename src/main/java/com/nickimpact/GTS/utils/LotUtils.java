...
public class LotUtils {
    ...
    public static void addPokemon4Pokemon(Player player,
                                          int slot,
                                          String note,
                                          PokeRequest pokemon,
                                          boolean expires,
    ...
    private static void addPokemonToMarket(int mode, Player player, int slot, String note, Object... options) {
        ...
        if (!listEvent.isCancelled()) {
            ...
            if (mode == 1) {
                ...
                textOptions.put("curr_symbol",
                                Optional.of(GTS.getInstance().getEconomy().getDefaultCurrency().getSymbol().toPlain()));
            ...
            try {
                ...
                if (account.isPresent()) {
                    ...
                    if (!purchase.isCancelled()) {
                        ...
                        if (acc.getBalance(
                                GTS.getInstance().getEconomy().getDefaultCurrency()).intValue() < price.intValue()) {
                            textOptions.put("money_diff", Optional.of(
                                    GTS.getInstance().getEconomy().getDefaultCurrency().getSymbol().toString() + (price.intValue() - acc.getBalance(
                                            GTS.getInstance().getEconomy().getDefaultCurrency()).intValue())));
                        ...
                        Optional<PlayerStorage> storage = PixelmonStorage.pokeBallManager.getPlayerStorageFromUUID(
                                (MinecraftServer) Sponge.getServer(), p.getUniqueId());
                        if (storage.isPresent()) {
                            storage.get().addToParty(pokemon);
                        } else {
                            GTS.getInstance().getLogger().error(
                                    "Player " + p.getName() + " was unable to receive a " + lot.getLot().getItem().getName() + " from GTS");
                        }

                        textOptions.put("pokemon", lot.getLot().getItem().getPokemon(lot.getLot()).isEgg ? Optional.of(
                                "Mystery Egg") : Optional.of(lot.getLot().getItem().getName()));
                        textOptions.put("curr_symbol", Optional.of(
                                GTS.getInstance().getEconomy().getDefaultCurrency().getSymbol().toPlain()));
                        textOptions.put("price", Optional.of(price));
                        textOptions.put("seller", Optional.of(
                                Sponge.getServiceManager().provideUnchecked(UserStorageService.class).get(
                                        lot.getLot().getOwner()).get().getName()));
                        ...
                        Optional<UniqueAccount> ownerAccount = GTS.getInstance().getEconomy().getOrCreateAccount(
                                lot.getLot().getOwner());
                        if (ownerAccount.isPresent()) {
                            ...
                            GTS.getInstance().getLogger().error(
                                    "Player '" + Sponge.getServiceManager().provideUnchecked(
                                            UserStorageService.class).get(
                                            lot.getLot().getOwner()).get().getName() + "' was unable to receive $" + price.intValue() + " from the GTS");
                        }
    ...
    public static void bid(Player player, Lot lot) {
        ...
        if (account.isPresent()) {
            ...
            if (acc.getBalance(GTS.getInstance().getEconomy().getDefaultCurrency()).intValue() < price.intValue()) {
                textOptions.put("money_diff", Optional.of(price.intValue() - acc.getBalance(
                        GTS.getInstance().getEconomy().getDefaultCurrency()).intValue()));
                textOptions.put("curr_symbol",
                                Optional.of(GTS.getInstance().getEconomy().getDefaultCurrency().getSymbol()));
        ...
        for (Player p : Sponge.getServer().getOnlinePlayers())
            if (!GTS.getInstance().getIgnoreList().contains(p.getUniqueId()) && !p.getUniqueId().equals(
                    player.getUniqueId())) {
                if (!p.getUniqueId().equals(lot.getHighBidder())) {
                    textOptions.put("player", Optional.of(player.getName()));
                    textOptions.put("seller", Optional.of(
                            Sponge.getServiceManager().provideUnchecked(UserStorageService.class).get(
                                    lot.getOwner()).get().getName()));
    ...
    public static void trade(Player player, LotCache lot, int slot) {
        ...
        Optional<PlayerStorage> owner = PixelmonStorage.pokeBallManager.getPlayerStorageFromUUID(
                (MinecraftServer) Sponge.getServer(), lot.getLot().getOwner());
        if (storage.isPresent() && owner.isPresent()) {
            EntityPixelmon pokemon = (EntityPixelmon) PixelmonEntityList.createEntityFromNBT(
                    storage.get().partyPokemon[slot], (World) player.getWorld());
            EntityPixelmon received = lot.getLot().getItem().getPokemon(lot.getLot());
            ...
            if (!trade.isCancelled() && !toLister.isCancelled()) {
                storage.get().removeFromPartyPlayer(slot);
                storage.get().addToParty(received);
                storage.get().sendUpdatedList();

                HashMap<String, Optional<Object>> variables = Maps.newHashMap();
                variables.put("pokemon", Optional.of(received.getName()));
                variables.put("player", Optional.of(
                        Sponge.getServiceManager().provideUnchecked(UserStorageService.class).get(
                                lot.getLot().getOwner()).get().getName()));
    ...
    public static void givePlayerPokemon(UUID uuid, Lot lot) {
        Optional<PlayerStorage> storage = PixelmonStorage.pokeBallManager.getPlayerStorageFromUUID(
                (MinecraftServer) Sponge.getServer(), uuid);
        if (storage.isPresent()) {
            storage.get().addToParty(lot.getItem().getPokemon(lot));
        } else {
            GTS.getInstance().getLogger().error(
                    "UUID (" + uuid + ") was unable to receive a " + lot.getItem().getName() + " from GTS");
        }
    }

    private static Optional<PlayerStorage> getStorage(Player player) {
        Optional<PlayerStorage> storage = PixelmonStorage.pokeBallManager.getPlayerStorageFromUUID(
                (MinecraftServer) Sponge.getServer(), player.getUniqueId());
        if (storage.isPresent())
    ...
    private static boolean checkPrice(Player player, EntityPixelmon pokemon, int price) {
        ...
        for (int iv : pokemon.stats.IVs.getArray()) {
            if (iv >= GTS.getInstance().getConfig().getMinIV()) {
                totalPrice += GTS.getInstance().getConfig().getMinIVPrice();
            }
        }
    ...
    private static boolean handleTax(Player player, EntityPixelmon pokemon, int price) {
        ...
        double t;
        if (pokemon.getIsShiny() && EnumPokemon.legendaries.contains(
                pokemon.getName()) && GTS.getInstance().getConfig().isStackTaxEnabled()) {
    ...
    public enum Commands {
        ...
        Update_Auction(
                "UPDATE `" + GTS.getInstance().getConfig().getMainTable() + "` SET Lot='{{lot}}' WHERE ID={{id}}"),
        Delete("DELETE FROM `" + GTS.getInstance().getConfig().getMainTable() + "` WHERE ID={{id}}"),
        Logs_Add(
                "INSERT INTO `" + GTS.getInstance().getConfig().getLogTable() + "`(ID, Date, uuid, Action, Log) VALUES ({{id}}, '{{Date}}', '{{UUID}}', '{{Action}}', '{{Log}}')");
