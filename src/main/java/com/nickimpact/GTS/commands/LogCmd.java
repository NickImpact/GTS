        ...
        if(target.isPresent()){
            ...
            player.ifPresent(pl -> {
                if(args.hasAny("r") && !args.hasAny("e")){
                    ...
                    for(Log log : logs){
                        Text text = Text.of(TextColors.GREEN, log.getId(), TextColors.GRAY, ") ",
                                TextColors.GOLD, log.getAction() + " ",
                                Text.builder().append(Text.of(TextColors.GRAY, "[", TextColors.YELLOW, "Log Info", TextColors.GRAY, "]"))
                                              .onClick(TextActions.runCommand("/gts:gts logs --log-id=" + log.getId()))
                                              .onHover(TextActions.showText(
                                                      Text.of(TextColors.WHITE, "Check out info on the pokemon!")
                                              ))
                                .build(), " ",
                    ...
                    PaginationList.builder()
                            .title(Text.of(TextColors.YELLOW, pl.getName() + "'s Logs"))
                            .contents(lText)
                            .sendTo(src);
                }
            });
